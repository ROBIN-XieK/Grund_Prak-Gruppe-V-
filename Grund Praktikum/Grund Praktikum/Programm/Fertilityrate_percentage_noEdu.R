...
plot_fertility_vs_schooling <- function(data, input_year) {
  ...
  scatter_plot <- ggplot(filtered_data, aes(x = Combind, y = FertilityRate, color = Region)) +
    ...
    labs(
      title = paste("Fertility Rate vs. percentage with no education (Year:", input_year, ")"),
      x = "percentage of female adults with no education",
      y = "Fertility Rate",
      color = "Region"
    ) +
    theme_minimal() +
    ...
    annotate(
      "text", 
      x = max(filtered_data$Combind, na.rm = TRUE) * 1.1,  # 横坐标最大值的右侧
      y = min(filtered_data$FertilityRate, na.rm = TRUE) - 0.4,        # 底部以下稍微偏移
